...
class Ion_auth
{
	...
	public function forgotten_password($identity)
	{
		if ($this->ion_auth_model->forgotten_password($identity)){
			...
			$user       = $this->where($identifier, $identity)->where('admin_status', '1')->users()->row();
			
			if ($user){
				$data = array(
					'identity' => $user->{$this->config->item('identity', 'ion_auth')},
					'admin_kode_lupapas' => $user->admin_kode_lupapas
				);
	...
	{
		$profile = $this->where('admin_kode_lupapas', $code)->users()->row(); // pass the code to profile
	...
	public function in_group($check_group, $id = FALSE, $check_all = FALSE){
		...
		if (isset($this->_cache_user_in_group[$id])){
			...
			$users_groups = $this->m_global->get('fokus_admin',$join,$where,$select);
			$groups_array = array();

			foreach ($users_groups as $group){
				$groups_array[$group->group_id] = $group->group_nama;
			}

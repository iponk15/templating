...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$query = $this->db->select('admin_password, admin_salt')
		                  ->where('admin_id', $id)
		                  ->limit(1)
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			if ($this->bcrypt->verify($password,$hash_password_db->admin_password))
			{
				return TRUE;
			}
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->db->where('admin_kode_lupapas', $code);

		if ($this->db->count_all_results($this->tables['users']) > 0){
			$data = array(
			    'admin_kode_lupapas'  => NULL,
			    'admin_waktu_lupapas' => NULL
			);
			
			$this->db->update($this->tables['users'], $data, array('admin_kode_lupapas' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('admin_id, admin_password, admin_salt')
		...
		$new = $this->hash_password($new, $result->admin_salt);

		// store the new password and reset the remember code so all remembered instances have to re-login
		// also clear the forgotten password code
		$data = array(
		    'admin_password'      => $new,
		    'admin_kode_ingat'    => NULL,
		    'admin_waktu_lupapas' => NULL,
		    'admin_waktu_lupapas' => NULL,
		);
	...
	public function forgotten_password($identity)
	{
		...
		$update = array(
			'admin_kode_lupapas' => $key,
			'admin_waktu_lupapas' => time()
		);
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array()){
		...
		// $query  = $this->db->get_where($this->tables['users_groups'], array('group_nama' => $this->config->item('default_group', 'ion_auth')), 1)->row();
		$where  = ['group_nama' => $this->config->item('default_group', 'ion_auth'),'group_status' => '1'];
		$select = 'group_id,group_nama,group_deskripsi';
		$getGrp = $this->m_global->get($this->tables['users_groups'],null,$where,$select);
		$query  = (empty($getGrp) ? NULL : $getGrp[0] );
		...
		$data = array(
			...
			'admin_username' 	=> $identity,
			'admin_password' 	=> $password,
			'admin_email' 		=> $email,
			'admin_ip' 	   	    => $ip_address,
			'admin_createddate' => date('Y-m-d H:i:s'),
			'admin_status'      => ($manual_activation === FALSE ? 1 : 0)
		);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select('admin_id,admin_email,admin_password,admin_status,admin_terakhir_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('admin_id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1){
			$user     = $query->row();
			$password = $this->hash_password_db($user->admin_id, $password);
			...
			if ($password === TRUE){
				...
				{
					$this->remember_user($user->admin_id);
				}
		...
		if ($recheck !== 0){
			...
			if ($last_login + $recheck < time())
			{
				$query = $this->db->select('admin_id')
								  ->where(array($this->identity_column => $this->session->userdata('identity'), 'admin_status' => '1'))
								  ->limit(1)
								  ->order_by('admin_id', 'desc')
								  ->get($this->tables['users']);
				if ($query->num_rows() === 1)
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$this->db->where('logatt_login', $identity);
			if ($this->config->item('track_login_ip_address', 'ion_auth'))
			{
				...
				$this->db->where('logatt_ip', $ip_address);
			}
			$this->db->where('logatt_waktu >', time() - $this->config->item('lockout_time', 'ion_auth'), FALSE);
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			$data = array('logatt_ip' => '', 'logatt_login' => $identity, 'logatt_waktu' => time());
			if ($this->config->item('track_login_ip_address', 'ion_auth'))
			{
				$data['logatt_ip'] = $this->_prepare_ip($this->input->ip_address());
			}
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$this->db->or_where('logatt_waktu <', time() - $old_attempts_expire_period, FALSE);
	...
	public function get_users_groups($id = FALSE){
		...
		// 				->get($this->tables['users_groups']);
		$join    = [
			['fokus_relasigroup','admin_id = regroup_userid','left'],
			['fokus_group','regroup_groupid = group_id','left']
		];

		$where   = ['admin_id' => $id];
		$select  = 'group_id,group_nama,group_deskripsi';
		$getData = $this->m_global->get('fokus_admin',$join,$where,$select)[0];
		$a       = json_decode(json_encode($getData), true);
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('admin_terakhir_login' => time()), array('admin_id' => $id));
	...
	public function set_session($user){
		...
		$session_data = array(
		    ...
		    'user_id'              => $user->admin_id, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       => $user->admin_terakhir_login,
		    'last_check'           => time(),
		);
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['users'], array('admin_kode_ingat' => $salt), array('admin_id' => $id));
	...
	public function login_remembered_user(){
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', admin_id, admin_email, admin_terakhir_login')
						  ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name', 'ion_auth'))))
						  ->where('admin_kode_ingat', get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array()){
		...
		$existing_group = $this->db->get_where($this->tables['users_groups'], array('group_nama' => $group_name))->num_rows();
		...
		$data = array('group_nama'=>$group_name,'group_deskripsi'=>$group_description);
		...
		$group_id = $this->db->insert_id($this->tables['users_groups'] . '_id_seq');
